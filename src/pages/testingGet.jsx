...
export default function TestingGet() {
  ...
  //   navigate(`/testingGet/${matricNo}`, { state: { matricNo, transactions } });
  // };

  const onCafeTransaction = (cafeId, transactions) => {
    navigate(`/testingGet/${cafeId}`, { state: { cafeId, transactions } });
  };
        ...
        <table className="w-full mx-auto text-center">
          <thead>
            <tr>
              ...
              <th className="text-center pb-[37px] font-medium">Actions</th>
            </tr>
          </thead>
          <tbody>
            {cafeTransactions.map((cafe, index) => (
              <tr key={index} className="text-gray-500">
                <td className="pb-6 pr-4 text-center">{cafe.id}</td>
                <td className="pb-6 text-left">{cafe.name}</td>
                <td className="pb-6 text-left">
                  {cafe.transaction.map((transaction, index) => (
                    <div key={index}>
                ...
                <td className="pb-6">
                  <button
                    ...
                    onClick={() =>
                      onCafeTransaction(cafe.cafeId, cafe.transaction)
                    }
                  >
                    Show Details
                  </button>
                ...
                <tr key={index} className="text-gray-500">
                  ...
                  <td className="pb-6">
                    <button
                      ...
                      className=" py-2 px-5 inline-flex justify-center items-center rounded-md border border-transparent font-semibold bg-[#C5c5c5] text-black hover:bg-[#Aaaaaa] focus:outline-none focus:ring-2 focus:ring-[#C5c5c5] focus:ring-offset-2 transition-all text-sm"
                      // onClick={() => {
                      //   // navigate(`/details/${student.matricNo}`);
                      // }}
                      onClick={() =>
                        onTransaction(student.matricNo, student.transaction)
                      }
